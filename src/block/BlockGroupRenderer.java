...
public class BlockGroupRenderer {
	...
	static {
		...
		Texture texture = new Texture("Heat Colour", Format.BGRA);
	...
	public void setupRender() {
		...
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, (IntBuffer) BufferUtils.createIntBuffer(array.length).put(array).flip(), GL15.GL_DYNAMIC_DRAW);
	...
	/** Allocates all of the default block textures */
	public static void allocateTextures() {
		allocateTexture("Embosed Sheet Blue");
		allocateTexture("Embosed Sheet");
		allocateTexture("Empty Grate");
		allocateTexture("Heating Grate Off");
		allocateTexture("Heating Grate On");
		allocateTexture("Hull Corner");
		allocateTexture("White Square");
	}
}
